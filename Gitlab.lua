...
local colors = {
    primary = Color3.fromRGB(255, 122, 41),
    secondary = Color3.fromRGB(138, 124, 255),
    accent = Color3.fromRGB(255, 158, 88),
    success = Color3.fromRGB(52, 211, 153),
    bg1 = Color3.fromRGB(13, 14, 18),
    bg2 = Color3.fromRGB(21, 23, 29),
    bg3 = Color3.fromRGB(31, 34, 43),
    bg4 = Color3.fromRGB(44, 48, 60),
    text = Color3.fromRGB(245, 246, 250),
    textDim = Color3.fromRGB(178, 184, 198),
    textDimmer = Color3.fromRGB(120, 126, 140),
...
local sidebarWidth = 124
local headerHeight = 36
local topBarHeight = 28
local sectionHeaderHeight = 30
local panelTransparency = 0.04
local sectionTransparency = 0.35
local fontSize = {
    title = 15,
    subtitle = 11,
    header = 12,
...
function Library:CreateWindow(config)
    ...
    new("UIStroke", {Parent = self._win, Color = colors.border, Thickness = 1, Transparency = 0.25})
    ...
    local function createMinimizedIcon()
        ...
        new("UIStroke", {Parent = icon, Color = colors.primary, Thickness = 1, Transparency = 0.4})
        new("TextLabel", {
            Parent = icon,
            Text = "L",
...
function Library:CreatePage(name, title, imageId, order)
    ...
    new("UIPadding", {Parent = contentContainer, PaddingTop = UDim.new(0, 2), PaddingBottom = UDim.new(0, 4), PaddingRight = UDim.new(0, 4)})
    ...
    self:AddConnection("navBtnHoverIn_" .. name, btn.MouseEnter:Connect(function()
        if self._currentPage ~= name then
            btn.BackgroundTransparency = 0.72
        end
    end))
    self:AddConnection("navBtnHoverOut_" .. name, btn.MouseLeave:Connect(function()
        if self._currentPage ~= name then
...
function Library:_switchPage(pageName)
    ...
    for name, data in pairs(self._navButtons) do
        ...
        data.btn.BackgroundColor3 = colors.bg3
        data.btn.BackgroundTransparency = isActive and 0.25 or 1
...
function Library:CreateCategory(parent, title, startOpen)
    ...
    new("UIStroke", {Parent = categoryFrame, Color = colors.border, Thickness = 1, Transparency = 0.55})
...
function Library:CreateToggle(parent, label, configPath, callback, disableSave, defaultValue)
    ...
    local toggleStroke = new("UIStroke", {Parent = toggleBg, Color = colors.border, Thickness = 1, Transparency = 0.45})
    ...
    local toggleTweenInfo = TweenInfo.new(0.16, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    local function updateVisual(instant)
        local bgColor = on and colors.primary or colors.bg3
        local circlePos = on and UDim2.new(1, -16, 0.5, -7) or UDim2.new(0, 2, 0.5, -7)
        local circleColor = on and Color3.fromRGB(255, 255, 255) or colors.textDim
        toggleStroke.Transparency = on and 1 or 0.45
        if instant then
            toggleBg.BackgroundColor3 = bgColor
            toggleCircle.Position = circlePos
            toggleCircle.BackgroundColor3 = circleColor
        else
            TweenService:Create(toggleBg, toggleTweenInfo, {BackgroundColor3 = bgColor}):Play()
            TweenService:Create(toggleCircle, toggleTweenInfo, {Position = circlePos, BackgroundColor3 = circleColor}):Play()
        end
    end
    updateVisual(true)
...
function Library:_createBaseDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValue, isMulti)
    ...
    new("UICorner", {Parent = selectFrame, CornerRadius = UDim.new(0, 5)})
    local selectStroke = new("UIStroke", {Parent = selectFrame, Color = colors.border, Thickness = 1, Transparency = 0.5})
    self:AddConnection("dropdownHoverIn_" .. dropdownLayoutOrder, dropdownButton.MouseEnter:Connect(function()
        selectStroke.Color = colors.primary
        selectStroke.Transparency = 0.25
    end))
    self:AddConnection("dropdownHoverOut_" .. dropdownLayoutOrder, dropdownButton.MouseLeave:Connect(function()
        selectStroke.Color = colors.border
...
function Library:CreateInput(parent, label, configPath, defaultValue, callback)
    ...
    local inputStroke = new("UIStroke", {Parent = inputBg, Color = colors.border, Thickness = 1, Transparency = 0.45})
    ...
    self:AddConnection("inputFocus_" .. label .. tostring(inputBox), inputBox.Focused:Connect(function()
...
function Library:MakeNotify(config)
    ...
    new("UIStroke", {Parent = notif, Color = colors.border, Thickness = 1, Transparency = 0.3})
